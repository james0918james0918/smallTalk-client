...
import { ADD_TEAM_FORM_FIELDS, FILE_TEAM_FROM_FIELD as logo } from '../../constants/index';
...
const normalInputGroups = () => ADD_TEAM_FORM_FIELDS.map(
  (item, index) => (
...
const fileInputGroup = (setFieldValue, logoFileObj) => (
  <div className="addTeamForm__inputGroup">
    <label htmlFor={logo.field} className={`addTeamForm__inputGroup__label addTeamForm__inputGroup__label--${logo.field}`}>{logoFileObj ? logoFileObj.name : logo.label}</label>
    <Field name={logo.field}
      type={logo.type}
      id={logo.field}
      placeholder={logo.placeholder}
      className={`addTeamForm__inputGroup__input addTeamForm__inputGroup__input--${logo.field}`}
      onChange={e => e.currentTarget.files[0] ? setFieldValue('logo', e.currentTarget.files[0]) : setFieldValue('logo', undefined)} />
  </div>
);
